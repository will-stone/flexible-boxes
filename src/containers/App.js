...
class App extends Component {
  ...
  sanitiseBoxes(boxes) {
    ...
    for (var compId in boxes) {
      if (boxes.hasOwnProperty(compId) && parseInt(compId, 10) === counter) { // second clause is dirty check for corrupt boxes; checks box IDs start at 1 and are squential.
        for (var key in boxes[compId]) {
          if (boxes[compId].hasOwnProperty(key)) {
            var value = boxes[compId][key];
            if (value === '') { // remove an empty property
  ...
  handleReorderBox(direction) {
    ...
    if (direction === 'up' && indexOfSelected !== 0) {
      ...
      if (boxes[boxes[parentId].c[indexOfSelected - 1]].c) {
        ...
        [boxes[parentId].c[indexOfSelected], boxes[parentId].c[indexOfSelected - 1]] = [boxes[parentId].c[indexOfSelected - 1], boxes[parentId].c[indexOfSelected]]
      }
    }
  ...
  handleDeleteBox(id, parentId) {
    ...
    if (parentsChildrenCount === 1) { // if only child, remove children (c) property from parent
      delete boxes[parentId].c;
    } else { // else just remove the child if present
  ...
  urlToBoxes() {
    if (window.location.hash) {
      var parsedBoxes;
      try { // check if parse-able otherwise reset
      ...
      if (parsedBoxes) { // successful parse
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <div dangerouslySetInnerHTML={browserWarning}></div>
        ...
        <div className={classnames(
          ...
          <div>
            <h1><i className="fa fa-warning fa-3x"/></h1>
            <h1>Flexible Boxes</h1>
            <p>This is a tool to help with creating Flexbox based website layouts. Due to all the toolbars and output boxes, it really does <strong>NOT</strong> work well with small screen sizes.</p>
            <p>Try maximising your browser or, if you are using a tablet, try turning it to landscape.</p>
            <p>If you would like to proceed anyway, please click <button onClick={this.removeScreenWarning.bind(this)}>here</button> (you have been warned).</p>
          </div>
        </div>
