    ...
    function buildUpCode(id, indentMultiplier) {
      ...
      if (boxes[id].c) {
        for (var i = 0; i < boxes[id].c.length; i++) {
          ...
          output += `${indent}<div class="fb fb__${id}_${compId}${boxes[compId].t ? '-' + boxes[compId].t : ''}">\n`;
...
${rootComment}${builtCode}${rootCompEnd}`;
    ...
    return (
      <div className="Html Pane__component">
        ...
        <SyntaxHighlighter language='html' style={atomOneDark} showLineNumbers={true}>{html}</SyntaxHighlighter>
      </div>
    );
  }
}
